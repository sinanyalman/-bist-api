...
// --- 1. TÜM TÜRKİYE PİYASASI (PARAMETRE DÜZELTMESİ) ---
async function getTurkeyAssets() {
    try {
        ...
        const body = {
            ...
            // DÜZELTME: |1d eklerini kaldırdık, standart veriyi istiyoruz.
            "columns": [
                "name",             // d[0]
                "close",            // d[1]
                "change",           // d[2] (Standart Değişim)
                "high",             // d[3] (Günün En Yükseği)
                "low",              // d[4] (Günün En Düşüğü)
                "description",      // d[5]
                "type",             // d[6]
                "subtype",          // d[7]
                "logoid",           // d[8]
        ...
        if (data && data.data) {
            data.data.forEach(item => {
                ...
                const asset = {
                    ...
                    price: d[1] || 0,
                    change24h: d[2] || 0, 
                    high24: d[3] || d[1], // Veri yoksa anlık fiyatı kullan
                    low24: d[4] || d[1],
                    mcap: d[9] || 0,
...
// --- 2. ABD HİSSELERİ (PARAMETRE DÜZELTMESİ) ---
async function getUSAssets() {
    try {
        ...
        const body = {
            ...
            "options": { "lang": "en" },
            // ABD için de standart parametrelere dönüyoruz
...
async function getForexAndGold() {
    try {
        ...
        const gramChange = ons.change; // Gram değişimi yaklaşık ons değişimi
        ...
        return [
            ...
            { id: 'GA', symbol: 'GRAM', name: 'Gram Altın', type: 'gold', price: gramPrice, change24h: gramChange, high24: gramPrice * 1.01, low24: gramPrice * 0.99, icon: 'gold', color: '#F1C40F' },
